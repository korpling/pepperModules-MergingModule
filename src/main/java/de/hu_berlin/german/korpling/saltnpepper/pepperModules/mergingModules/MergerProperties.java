...
public class MergerProperties extends PepperModuleProperties {
	...
	{
		this.addProperty(new PepperModuleProperty<String>(PROP_MERGING_LEVEL, String.class, "Determines the level of merging, the level can be one of the following types '"+MERGING_LEVEL.MERGE_CORPUS_GRAPHS+"' just copies all corpus graphs to a single one, '"+MERGING_LEVEL.MERGE_DOCUMENT_PATHES+"' merges all corpus graphs into a single corpus graph, but does not merge the documents and '"+MERGING_LEVEL.MERGE_DOCUMENTS+"' merges all possible documents (by given table or autodetect)", true));
	}
	
	public MERGING_LEVEL getMergingLevel(){
		MERGING_LEVEL retVal= null;
		String level= getProperty(PROP_MERGING_LEVEL).getValue().toString();
		if (	(level!= null)&&
				(!level.isEmpty()))
